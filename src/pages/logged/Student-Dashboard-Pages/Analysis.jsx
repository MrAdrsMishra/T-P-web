...
const Analysis = () => {
  ...
  const {
    ...
    fetchSkillsClassification,
    fetchMetricTree,
    fetchHierarchicalPerformance,
  } = useAnalyticsStore();
  ...
  useEffect(() => {
    if (token) {
      ...
      fetchSkillsClassification(token);
    }
  }, [token, fetchMetricTree, fetchHierarchicalPerformance, fetchSkillsClassification]);

  const weakTopics = skillsClassification?.weakSkills || [];
  const strongTopics = skillsClassification?.strongSkills || [];
    ...
    <div className="flex flex-col space-y-4 p-4 md:p-6">
      ...
      <div className="bg-white p-6 rounded-xl shadow border border-gray-100">
        <h3 className="text-lg font-bold text-gray-900 mb-4 flex items-center gap-2">
          🎯 Metric & Topic Skills Diagnostics
        </h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-red-50/60 border border-red-200 rounded-lg p-4">
            <h4 className="text-sm font-semibold text-red-800 mb-3 flex items-center justify-between">
              <span>⚠️ Weak Skills (&lt; 60% Accuracy)</span>
              ...
              <div className="space-y-2 max-h-80 overflow-y-auto">
                {weakTopics.map((item, idx) => (
                  <div key={idx} className="flex items-center justify-between bg-white p-2.5 rounded border border-red-100 shadow-sm">
                    <div>
                      <span className="font-semibold text-gray-800 text-sm">{item.name || item.tag}</span>
                      <span className="text-xs text-gray-500 block">
                        Accuracy: {item.accuracy}%
                      </span>
                    </div>
                    <button
                      onClick={() => navigate(`/practice?tag=${encodeURIComponent(item.name || item.tag)}`)}
          ...
          <div className="bg-emerald-50/60 border border-emerald-200 rounded-lg p-4">
            <h4 className="text-sm font-semibold text-emerald-800 mb-3 flex items-center justify-between">
              <span>🌟 Strong Skills (&gt; 80% Accuracy)</span>
